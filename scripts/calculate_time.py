...
parser.add_option("--num-scores",dest="num_scores")
parser.add_option("--runtimes",dest="runtimes")
...
elif ( options.num_scores == None ):
	error("specify --num-scores")
elif ( options.runtimes == None ):
	error("specify --runtimes")
...
#			cat=cat[0:str.find(cat,'.')]
	...
	for tool in svcomp_tools:
		cmd="cut -f 1 -d ' ' {0}_{1} > {2}_test_files; tail -n +2 {3} | cut -f 1,{4} -d ' ' > {2}_tool_scores_{4}; LC_ALL=C join {2}_test_files {2}_tool_scores_{4} | cut -f 2 -d ' ' | tr '\n' ' '".format(options.prediction_file,exnum,tmp,options.num_scores,tool+1)
		tool_scores.append([int(x) for x in subprocess.check_output(cmd,shell=True).split()])

		cmd="cut -f 1 -d ' ' {0}_{1} > {2}_test_files; tail -n +2 {3} | cut -f 1,{4} -d ' ' > {2}_tool_runtimes; LC_ALL=C join {2}_test_files {2}_tool_runtimes | cut -f 2 -d ' ' | tr '\n' ' '".format(options.prediction_file,exnum,tmp,options.runtimes,tool+1)
		tool_runtimes.append([float(x) for x in subprocess.check_output(cmd,shell=True).split()])
...
def get_score_runtime(tool, num):
	answer=''
...
#		answer=succ_fail_to_str(SuccFailType.unknown)
	...
	#	print('get_score_runtime tool={0} num={1}'.format(tool,num))
	#	print('len(tool_scores)={0}\n'.format(len(tool_scores)))
	#	print('len(tool_scores[{0}])={1}\n'.format(tool-1,len(tool_scores[num])))

		num_score = tool_scores[tool-1][num]
		runtime = tool_runtimes[tool-1][num]

		if num_score > 0:
			succ_fail=SuccFailType.succ
		elif num_score == 0:
			succ_fail=SuccFailType.unknown
		else:
			succ_fail=SuccFailType.fail
...
#		add_tool_result(MetaToolsType.perfect_ps22, cat, num_score, runtime, succ_fail)
		...
		if options.debug:
			flog.write('{0} ps_tool={1} score={2} rt={3} place={4} cat_tool_scores[({5},{6},{7})]={8}\n\n'.format(filename, get_tool_name(ps_tool), ps_num_score, ps_runtime, ps_place, get_cat_name(cat),get_tool_name(MetaToolsType.ps),ex,cat_tool_scores[(cat,MetaToolsType.ps,ex)]))
			...
			if not ( is_excluded_tool(tool) ):
				...
				if ( ex == EXSUM ):
					ftable_tool_places.write("\t{0} {1:.0f} {2:.0f}".format(cat_tool_places[(cat, tool)],
																				cat_tool_scores[(cat, tool, ex)],
																				cat_tool_runtimes[(cat, tool, ex)]/60))
